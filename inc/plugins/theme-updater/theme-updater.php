 ...
 */
if ( ! function_exists( 'seese_get_current_theme_update' ) ) {
  function seese_get_current_theme_update( $updates ) {

    if ( isset( $updates->checked ) ) {

      if ( ! class_exists( 'Envato_Protected_API' ) ) {
        require_once( SEESE_FRAMEWORK .'/plugins/theme-updater/class-envato-protected-api.php' );
      }
  ...
  add_filter( 'pre_set_site_transient_update_themes', 'seese_get_current_theme_update' );
}

if ( ! function_exists( 'seese_admin_head' ) ) {
  function seese_admin_head() {
  ...
  add_action( 'admin_head', 'seese_admin_head' );
}